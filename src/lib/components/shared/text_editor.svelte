        ...
        <div class="flex place-items-center gap-2 pr-4 md:gap-[0.75vw] md:pr-[1vw]">
            {#each Object.entries(icon_and_function_mapping) as item}
                ...
                <button
                    ...
                    use:tippy={{
                        content: `<div class='leading-2 w-max whitespace-nowrap rounded-lg bg-surface-400 px-3 py-2 text-xs text-surface-50 md:px-[0.625vw] md:py-[0.42vw] md:text-[1vw]'>${xss(description)}</div>`,
                        allowHTML: true,
                        arrow: false,
                        offset: [0, 17],
