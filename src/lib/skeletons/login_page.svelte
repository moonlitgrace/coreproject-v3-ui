<form class="flex h-max w-full flex-col bg-surface-900 p-10 pb-[10vw] pt-[7vw] md:h-full md:justify-between md:rounded-none md:p-0">
    <div class="placeholder mb-[5vw] flex h-6 w-60 items-center !bg-surface-400 md:mb-[1vw] md:w-[58%]" />
    <div>
        <div>
            <div class="placeholder mb-1.5 h-6 w-36 !bg-surface-400" />
            <div class="placeholder mt-[0.25vw] h-11 w-full !bg-surface-400 md:h-[3.125vw]" />
            <div class="placeholder mt-1.5 w-[90%] !bg-surface-400" />
        </div>
        <password-field>
            <div class="placeholder mb-1.5 mt-[2vw] h-6 w-24 !bg-surface-400 md:mt-[1.5vw]" />
            <div class="placeholder mt-[0.25vw] h-11 w-full !bg-surface-400 md:h-[3.125vw]" />
        </password-field>
        <div class="mt-5 flex flex-col items-start md:mt-[3vw]">
            <div class="placeholder h-6 w-[40%] !bg-surface-400" />
        </div>
    </div>
    <div class="mt-10 flex items-center justify-between md:mt-0">
        <div class="flex flex-col gap-1">
            <span class="placeholder h-5 w-36 !bg-surface-400" />
            <span class="placeholder h-5 w-12 !bg-surface-400" />
        </div>
        <div class=" placeholder h-12 w-[32.8%] !bg-surface-400 p-0 px-5 md:h-[2.75vw] md:px-[1.25vw]" />
    </div>
</form>
